...
def drink_db_view(request):
    ...
    # Drinks = DrinkModel.objects.order_by('id')
    # drink = DrinkModel.objects.filter(category_name='맥주')
    # print(Drinks)
    # chicken = FoodModel.objects.filter(category_name='닭,치킨,Chicken')
    # print(chicken)
    # food = FoodModel.objects.all()
    # food_title = food[3].title
    # print(food_title)
