...
id_num = sub_scraper(Makgeolli, "막걸리", id_num, 'Makgeolli', 2)
...
driver.quit()
#
# ros_wine = soup.select_one(
#     '#container_w20210529ef64b0e736f1c')
# sub_scraper(ros_wine, "로제, 스파클링 와인", id_num)


#
# def sub_scraper(category, category_name, id_num):
#     i = 0
#     for tr in category:
#         i += 1
#         id_num += 1
#         try:
#             img = tr.select_one('div:nth-child(' + str(i) + ') > div.item-wrap > a > img')['data-original']
#             title = tr.select_one('div:nth-child(' + str(i) + ') > div.item-detail > div.item-pay > h2 > a').text
#             price = tr.select_one('div:nth-child(' + str(i) + ') > div.item-detail > div.item-pay > '
#                                                               'div.item-pay-detail > p.pay.inline-blocked').text
#             description = tr.select_one('div:nth-child(' + str(i) + ') > div.item-detail > '
#                                                                     'div.item-summary.holder.fr-view > a').text
#         except Exception as e:
#             continue
#
#         else:
...
# sub_scraper(red_wine, "레드 와인", id_num)
#
# driver.quit()
